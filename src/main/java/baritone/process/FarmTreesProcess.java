...
public final class FarmTreesProcess extends BaritoneProcessHelper implements IFarmTreesProcess {
    ...
    private PathingCommand handleChopping(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (logs.isEmpty()) {
            // No more logs — move to replanting phase
            if (Baritone.settings().farmTreesReplantSaplings.value) {
                phase = Phase.REPLANTING;
                return handleReplanting(false, isSafeToCancel);
            } else {
                // Skip replanting, go straight to waiting
                phase = Phase.WAITING;
                waitTicksRemaining = Baritone.settings().farmTreesWaitTicks.value;
                logDirect("Tree farm area clear, waiting " + (waitTicksRemaining / 20) + "s for regrowth...");
                return new PathingCommand(null, PathingCommandType.REQUEST_PAUSE);
            }
        }
        ...
        // Try to break a log within reach first
        ...
        if (calcFailed) {
            ...
            // Don't cancel, just retry
        }

        // Path to the highest log blocks
    ...
    private PathingCommand handleReplanting(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (plantable.isEmpty() || !hasSaplingInInventory()) {
            // Nothing to plant or no saplings available, go to waiting
            phase = Phase.WAITING;
            waitTicksRemaining = Baritone.settings().farmTreesWaitTicks.value;
            logDirect("Tree farm area clear, waiting " + (waitTicksRemaining / 20) + "s for regrowth...");
            return new PathingCommand(null, PathingCommandType.REQUEST_PAUSE);
        ...
        if (calcFailed) {
            // Can't reach plantable spots, move to waiting
            phase = Phase.WAITING;
            waitTicksRemaining = Baritone.settings().farmTreesWaitTicks.value;
            logDirect("Tree farm area clear, waiting " + (waitTicksRemaining / 20) + "s for regrowth...");
            return new PathingCommand(null, PathingCommandType.REQUEST_PAUSE);
    ...
    private PathingCommand handleWaiting() {
        waitTicksRemaining--;
        if (waitTicksRemaining <= 0) {
            logDirect("Rescanning tree farm area...");
